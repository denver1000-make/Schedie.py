...
def turn_on_proc(
    ...
    cancellation_info = check_if_timeslot_cancelled(
...
def turn_off_proc(
    ...
    cancelled_schedule_info = check_if_timeslot_cancelled (
        timeslot_id=timeslot_id
    )
    
    if cancelled_schedule_info:
        print(f"Cancellation Detected for Timeslot {timeslot_id}")
        remove_cancelled_info_orm_by_timeslot_id(
            timeslot_id=timeslot_id
        )
        return
    ...
    if nearby_timeslot:
        ...
        # Check if nearby schedule is cancelled
        is_nearby_slot_cancelled = check_if_timeslot_cancelled(
            nearby_timeslot.timeslot_id
        )
